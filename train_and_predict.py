...
PREDICTIONS_DB = 'data/predictions.db'  # SQLite DB for predictions in 'data' folder
...
def main():
    ...
    for table in tables:
        ...
        X = df.drop(columns=['Date', 'targetn7d'])  # Replace 'target' with your target column name
        y = df['targetn7d']  # Replace 'target' with your target column name
        ...
        predictions_df = pd.DataFrame({'date': dates, 'actual': df['target']})
